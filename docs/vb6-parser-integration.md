...
- 已支援 CLI 參數 `--parser=ast`，並可檢查 Proleap 類別是否已加入 classpath。

> 注意：目前 `.frm` 抽取器仍屬 MVP 版本（以 regex 解析），後續可替換為 AST-based 抽取。

## AST 模式啟用方式（待整合 Proleap 原始碼）

- CLI 已支援 `--parser=ast` 參數，但需先把 Proleap VB6 Parser 放入 classpath。
- 若尚未 vendor Proleap，AST 模式會提示缺少 class。
- 目前 AST 模式仍使用 regex 抽取邏輯，待 AST walker 完成後才會產生真正 AST-based 抽取結果。
...
- 建立 AST walker（Form/Module/Class）
- 以 AST 取代 `FrmExtractor` / `BasExtractor` 的 regex 邏輯
- 補齊 `.cls` / `.ctl` 的解析與關係抽取
